...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            print(
                "******************* DATASET: {} ****************".format(dataset_key)
            )
            tps, fns, fps = 0, 0, []
            for idx, data in enumerate(dataset):
                inputs, masks, neg_masks = (
                ...
                for i in range(nr_batches):
                    ...
                    x_i = inputs[i][0]
                    x_rec_i = (
                        reconstructions[i][0] if reconstructions is not None else None
                    )
                    ...
                    if x_rec_i is not None:
                        ...
                        loss_lpips = np.squeeze(
                            lpips_alex(x_i.cpu(), x_rec_i.cpu()).detach().numpy()
                        )
                        test_metrics["LPIPS"].append(loss_lpips)
                        x_rec_i = x_rec_i.cpu().detach().numpy()
                        ssim_ = ssim(
                            x_rec_i, x_i.cpu().detach().numpy(), data_range=1.0
                        )
                    ...
                    tp = (
                        1 if amount_anomaly > 0.1 * amount_mask else 0
                    )  # 10% overlap due to large bboxes e.g. for enlarged ventricles
                    tps += tp
                    fn = 1 if tp == 0 else 0
                    fns += fn
                    ...
                    test_metrics["F1"].append(
                        2 * (precision * tp) / (precision + tp + 1e-8)
                    )
                    ...
                    if int(count) == 0:
                        ...
                        diffp, axarr = plt.subplots(
                            1, len(elements), gridspec_kw={"wspace": 0, "hspace": 0}
                        )
                        ...
                        for idx_arr in range(len(axarr)):
                            ...
                            axarr[idx_arr].imshow(
                                np.squeeze(elements[idx_arr]),
                                vmin=0,
                                vmax=v_max,
                                cmap=c_map,
                            )
            ...
            for metric in test_metrics:
                ...
                if metric == "TP":
                    print(
                        f"TP: {np.sum(test_metrics[metric])} of {len(test_metrics[metric])} detected"
                    )
                if metric == "FP":
